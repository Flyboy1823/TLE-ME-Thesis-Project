...
time_tol=minutes(10);  % [duration] - no need to interpolate if the time difference between the two observations is less than 1 minute
a_tol=3000; % [meters]
i_tol=2; %[deg]
e_tol=.05;
a_tle=[];
...
for counter=2:round(tle_len)  
    ...
    if (Epoch_b-Epoch_a) > time_tol
        ...
        time_interpolated=[time_interpolated, interpolate_time(1:(end-1))];
        %[temp_prop_positions,temp_prop_velocities] = propagateOrbit(interpolate_time,a,e,i,RAAN,om,tru);
        ...
        time_interpolated=[time_interpolated, Epoch_a];
...
grid on
    ...
    % --- Optional: Visualize the orbit (requires Aerospace Toolbox or custom plotting) ---
    % If you have the Aerospace Toolbox, you can use orbitPlot.
    % figure;
    % orbitPlot(r_eci, 'CoordinateFrame', 'ECI');
