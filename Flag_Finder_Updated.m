function [flag, time_tle, r_output, v_output] = Flag_Finder_Updated(SatelliteID)
    ...
    for counter=2:round(tle_len)  
        ...
        if (Epoch_b-Epoch_a) > time_tol
            interpolate_time = [Epoch_a, dateshift(Epoch_a, 'end', 'minute'):time_tol:dateshift(Epoch_b, 'start', 'minute'), Epoch_b];
            if isempty(time_interpolated)
                time_interpolated=[interpolate_time];
            else
                time_interpolated=[time_interpolated(1:(end-1)), interpolate_time];
            end
            
            %[temp_prop_positions,temp_prop_velocities] = propagateOrbit(interpolate_time,tleData_a);
            [temp_prop_positions,temp_prop_velocities] = Orbit_Prop_Test(r,v,time_interpolated);
            
            if isempty(r_interpolated)
                r_interpolated=[temp_prop_positions];
                %v_interpolated=[temp_prop_velocities];
            else
                r_interpolated=[r_interpolated(:,1:(end-1)), temp_prop_positions];
            ...
            r_interpolated=[r_interpolated, r];
    ...
    r_output = r_interpolated';
    v_output = v_interpolated';
end
